...
const fetchTopTokens = async (chainName: MultiChainName, timestamp24hAgo: number): Promise<string[]> => {
  const whereCondition = `where: { date_gt: ${timestamp24hAgo}, token_not_in: $blacklist, dailyVolumeUSD_gt:2000 }`
    // chainName === 'ETH'
...
const useTopTokenAddresses = (): string[] => {
  ...
  const fetch = useCallback(async () => {
    const addresses = await fetchTopTokens("ARB", timestamp24hAgo)
