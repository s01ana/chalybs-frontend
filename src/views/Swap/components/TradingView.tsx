...
import { Box, FlexProps } from 'components'
...
const initializeTradingView = (TradingViewObj: any, theme: DefaultTheme, opts: any) => {
...
const TradingView = ({ id, symbol }: TradingViewProps) => {
  ...
  useEffect(() => {
    ...
    if (window.tv) {
      // @ts-ignore
      initializeTradingView(window.tv, theme, opts)
    } else {
      tradingViewListener().then((tv) => {
        initializeTradingView(tv, theme, opts)
      })
    }
