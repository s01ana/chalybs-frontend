import { Flex, Input, Skeleton, Text } from 'components'
...
const Search = () => {
  ...
  const tokensForList = useMemo(() => {
    if (watchListTokenData && showWatchlist) {
  ...
  const poolForList = useMemo(() => {
    if (showWatchlist && watchListPoolData) {
    ...
    return (
      <>
        ...
        {!showWatchlist && debouncedSearchTerm.length < MINIMUM_SEARCH_CHARACTERS && (
          <Text>Search liquidity pairs or tokens</Text>
        )}
      </>
    )
  }
            ...
            {tokensForList.slice(0, tokensShown).map((token) => {
              return (
                ...
                >
                  <ResponsiveGrid>
                    <Flex>
                      <CurrencyLogo address={token.address as `0x${string}`} chainName="ARB" />
                      <Text ml="10px">
              ...
              return (
                ...
                >
                  <ResponsiveGrid>
                    <Flex>
                      <DoubleCurrencyLogo
                        address0={p.token0.address as `0x{string}`}
                        address1={p.token1.address as `0x{string}`}
                        chainName="ARB"
                      />
